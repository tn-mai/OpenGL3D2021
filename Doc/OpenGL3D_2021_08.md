...
<p align="center">
<img src="images/08_turbosquid_show_all.png" width="66%" />
</p>
...
<p align="center">
<img src="images/08_turbosquid_download_obj_and_tex.png" width="50%" />
</p>

このモデルはOBJファイルとテクスチャが個別のファイルに分けられているようです。OBJファイルとテクスチャの2つのファイルをダウンロードして、プロジェクトの`Res`フォルダにコピーしてください。

<pre class="tnmai_assignment">
<strong>【課題11】</strong>
ダウンロードしたテクスチャをTGAファイルに変換しなさい。
</pre>

<pre class="tnmai_assignment">
<strong>【課題12】</strong>
ダウンロードした<code>HouseRender.obj</code>を、座標<code>(-8, 2, 0)</code>に表示しなさい。できるだけ他のモデルと重ならない位置に表示すること。
</pre>
...
オブジェクト名から考えて、おそらく4行目の`Plane`が背景パーツでしょう。試しに4行ほどコメントにしてみましょう。`HouseRender.obj`の297行目付近を次のように変更してください。
...
拡大しただけなのに建物が移動したのは、OBJファイルの時点で原点に配置されていないからです。これを解決するには、拡大縮小のまえに「物体を原点に平行移動」します。

OBJファイルをグラフィックスデザイナーで開いて調べたところ、建物から原点までの距離は`(-2.6, 2.0, 0.8)`でした。この距離を平行移動させる行列を追加すれば良さそうです。建物を表示するプログラムを次のように変更してください。
