...
>**【テキストを始めるまえに】**<br>
>今回は数学的な話が多いので、難しく感じることが多いと思います。<br>
>内容が分からなくてもあまり気にせずに、とにかくプログラムを書いていってください。
...
アクターを「重なっていない状態」に戻す簡単な方法のひとつは「移動前の座標に戻す」ことです。しかし、この方法では「物体にそって滑らせる」ことができません。



より妥当な方法は「重なっている部分の長さ」を求めて、その長さだけアクターを移動させることです。
...
それでは「重なっている部分の長さ」を求めましょう。`Actor.h`を開き、`DetectCollision`関数を次のように変更してください。
...
実際に重ならないようになっているかを調べるために、タイガーI戦車にもコライダーを設定しましょう。`Main.cpp`を開き、戦車を作成するプログラムに次のプログラムを追加してください。
...
コンタクト構造体の情報を使って、重なりを解決する関数を定義します。関数名は`SolveContact`(ソルブ・コンタクト)とします。`Solve`(ソルブ)は「解決する」、`Contact`(コンタクト)は「接触」という意味の英単語です。
...
プログラムが書けたらビルドして実行してください。建物を押しても建物が動かなければ成功です。
...
プログラムが書けたらビルドして実行してください。発射した弾が、斜め上に飛んでいくことはなくなるはずです。それから、戦車をエレベーターに乗せてみてください。戦車が下に降りてこられたら成功です。
...
プログラムを起動してから1分くらい放置すると、アクターが地面を突き抜けて落ちていきます。これは重力が加算され続けた結果、ベロシティの数値が非常に大きくなってしまったためです。
...
まずはアクターに質量と反発係数を設定できるようにしましょう。変数名には英訳を使います。質量を英訳すると`mass`(マス)です。反発係数を英訳すると`coefficient of restitution`(コエフィシエント・オブ・リスティテューション)ですが、長すぎるので`cor`(コー)とします。
...
それと、衝突後の速度を計算するとベロシティの値が変化します。しかし、同時に複数の衝突が起きた場合、衝突を処理する順番によってベロシティが違ってしまうと正しく計算ができません。
...
浸透方向が分かっているので、その逆方向を法線候補としています。次に、スコア変数を作成します。`Actor.cpp`を開き、ディテクトコリジョン関数に次のプログラムを追加してください。
...
本テキストではUE4やUnityの初期値である「平均値」を使うことにしました。

非弾性衝突による衝突後の速度の計算には、Wikipediaの「非弾性衝突」のページにある以下の公式を使います。
...
>**【内積と射影】**<br>
>ベクトルBからベクトルAに垂直に光を当てたとき、ベクトルA上にできるベクトルBの影の長さを求めることを「射影(しゃえい)」といいます。射影は「法線方向の速度を求める」というような問題を解くために使われます。<br>
>内積は射影を効率的に計算する手段です。
...
+    float va = (actorA.mass * ua + actorB.mass * ub + cor * actorB.mass * (ub - ua)) / massAB;
+    float vb = (actorA.mass * ua + actorB.mass * ub + cor * actorA.mass * (ua - ub)) / massAB;
...
まずは、アクター構造体に摩擦係数の変数を追加します。変数名は摩擦の英訳である`friction`(フリクション)にします。`Actor.h`を開き、次のプログラムを追加してください。
...
タンジェントを求めるためには「外積(がいせき)」という計算を行います。外積は

>2つのベクトルに対して垂直なベクトル

を求めることができます。なんだかややこしそうですが、GLMライブラリには外積を計算する`cross`(クロス)関数があるので難しく考えなくても大丈夫です。
...
別の方法として、「似ているコンタクト構造体を併合する」というやりかたがあります。この方法では、同じ平面にどれだけコライダーが重なっていたとしても、それらの衝突は併合されて1回の衝突にまとめられます。

その結果、何度も押し返されることがなくなるというわけです。今回はコンタクト構造体を併合する方法を使います。そして、以下の条件を満たした場合に「似ている」と判定することにします。
...
イコール関数を使ってコンタクト構造体を併合するプログラムを書いていきます。`Main.cpp`を開き、コンタクト構造体を配列に追加するプログラムを次のように変更してください。
...
作成するのは、「浸透が始まった時間」に応じてスコアを加算する処理です。アクターAが秒速`V`でアクターBに衝突したとします。このときの浸透距離を`P`とすると、浸透が始まったのは`P / V`秒前だと考えられます。

軸の方向によって速度も浸透距離も異なるので、浸透が始まった時間も異なります。そして「もっとも早く浸透を始めた方向 = 衝突した方向」考えるのは妥当でしょう。

それではプログラムを書いていきます。まずアクターAがアクターBに衝突する速度を求めます。`Actor.cpp`を開き、衝突面のスコアを計算するプログラムに次のプログラムを追加してください。
...
次に浸透が始まった時間を求めます。これは、すべての軸について`P / V`を行うだけです。このときも、`0`除算が起きないように注意しなくてはなりません。

相対ベロシティを計算するプログラムの下に、次のプログラムを追加してください。
...
プログラムが書けたらビルドして実行してください。地面に引っかかることが少なくなっているはずです(ゼロではないですが)。
...
衝突判定が完成したので、建物や木にコライダーを設定してちょっとした迷路を作ってみましょう。ただ、10x10のマップは迷路を作るにはちょっと狭いと思いますので、マップを大きくしましょう。
...
マップを大きくすると処理速度が大きく低下すると思います。これは衝突判定の回数が大幅に増加したからです。しかし、CPUが本来の能力を発揮できていれば、この程度の判定回数は問題にならないはずです。

すぐに処理速度が落ちてしまう原因は、プロジェクトのプロパティ設定にあります。ソリューションエクスプローラーのプロジェクト名を右クリックし「プロパティ」を選択してください。
...
設定を変更したら、ビルドして実行してください。処理速度が戻っていると思います。
...
そして、タイガーI戦車のいる方向との角度差を求めます。角度差を求めるには「内積」と`acos`(エー・コス、アーク・コサイン)関数を使います。ベクトルdを正規化するプログラムの下に、次のプログラムを追加してください。
...
>単位ベクトル同士の内積の結果を`acos`関数に渡して、ベクトルのなす角`θ`を求めている
...
内積を使うと`cosθ`が得られるように、外積を使うと`sinθ`が得られることが分かると思います。外積の結果はベクトルになりますが、この「結果のベクトルの長さ」が`sinθ`に当たります。
