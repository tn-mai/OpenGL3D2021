...
+  vec3 prevPosition = mix(vPrevBaseMeshPosition, vPrevMorphTargetPosition, morphWeight.y);
+  vec3 position = mix(curPosition, prevPosition, morphWeight.z);
+
-  vec3 normal = (1 - morphWeight) * vNormal + morphWeight * vMorphNormal;
+  vec3 curNormal = mix(vNormal, vMorphNormal, morphWeight.x);
+  vec3 prevNormal = mix(vPrevBaseMeshNormal, vPrevMorphTargetNormal, morphWeight.y);
...
`GLContext.h`を開き、`SetMorphTargetMesh`関数の宣言の下に、次の関数宣言を追加してください。
...
追加した引数にもデフォルト引数として`nullptr`を設定しておきます。理由は、`morphTarget`引数を追加したときと同じで、モーフィングを必要としない描画の場合にこれらの引数の指定を不要にするためです。
...
>* ユニフォーム変数を増やすとき、既存の変数の型を変えることで対応できる場合がある。
