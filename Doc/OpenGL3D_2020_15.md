...
<p align="center">
<img src="images/15_cursor_image.png" width="25%" />
</p>
...
<p align="center">
<img src="images/15_window_coord_to_clip_coord.png" width="75%" /><br>
...
>増やした次元の初期値には適当に都合のいい値を設定します。2D座標系をZ軸回転させる場合、増やしたZ座標の値を`0`にしておくと意図した変換結果にしやすいです。同様に3Dの座標変換では増えた次元を使って平行移動を行いますが、この場合は増えた次元を`1`にすると意図した結果にしやすいのです。

### 1.7 線分と平面の交差判定

1.6節で求めた2点を結ぶ<ruby>線分<rt>せんぶん</rt></ruby>と、プレイヤーの腰の高さにあるXZ平面との交差判定を作っていきます。まず直線および平面をあらわす構造体を定義しましょう。衝突判定は `Actor.h` と `Actor.cpp`に書いているので、線分と平面の交差判定も`Actor.h`に定義することにします。
...
まずはゾンビの攻撃アニメーションを用意します。ウェブブラウザで<br>
`https://github.com/tn-mai/OpenGL3D2020/tree/master/Res/zombie_male/`<br>
...
ただし、まだゾンビの攻撃アニメーションは表示されません。なぜかというと、攻撃アニメーションを毎フレーム設定し直しているので、再生状態がリセットされてしまうからです。
...
ただし、これらの回避策はどれも注意深くプログラムしなければ正しく動きません。そのため、
...
それでは、アクターの状態を更新する`for`ループの下に、次のプログラムを追加してください。
...
次に、攻撃判定が残り続けないように、攻撃範囲アクターを削除する必要があります。アニメ番号が4のときを判定するプログラムに、次のプログラムを追加してください。
...
ウェブブラウザで<br>
`https://github.com/tn-mai/OpenGL3D2020/tree/master/Res/player_male/`<br>
にアクセスし、以下のファイルをダウンロードしてプロジェクトの`Res`フォルダに保存してください。
...
>プレイヤーが死ぬと死亡アニメーションが再生される<br>
>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;↓<br>
>死亡アニメーションが終了したらゲームオーバーフラグが立つ<br>
>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;↓<br>
>ゲームオーバーフラグが立っていたらEnterキーでタイトル画面に戻ることができる
...
まずはゾンビのダメージアニメーションを用意します。ウェブブラウザで<br>
`https://github.com/tn-mai/OpenGL3D2020/tree/master/Res/zombie_male/`<br>
にアクセスし、以下のファイルをダウンロードしてプロジェクトの`Res`フォルダに保存してください。
...
そこで、`Collision`構造体に「相手を押し返す機能」の有無を示すメンバ変数を追加し、この変数が`false`ならアクターを押し返さない、というプログラムを追加することにします。
...
次に、衝突判定関数に「ブロックするかしないか」を引数として渡せるようにします。 `Actor.cpp`を開き、`CollideCylinders`関数の定義を次のように変更してください。
...
それでは、`DetectCollision`関数を次のように変更してください。
