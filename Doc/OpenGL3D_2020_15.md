...
<pre class="tnmai_assignment">
<strong>【課題02】</strong>
ゾンビの攻撃アニメーションを作成しなさい。変数名は<code>anmZombieMaleAttack</code>とします。また、インターバルは<code>0.125f</code>、ループ再生は<code>false</code>に設定してください。
</pre>
...
次に、プレイヤーアクターの耐久値を設定します。とりあえず`10`にしましょう。`MainGameScene.cpp`を開き、`Initialize`メンバ関数のプレイヤーアクターを作成するプログラムに、次のプログラムを追加してください。
...
### 3.4 DetectCollision関数をブロックフラグに対応させる
...
マウスのボタンによる射撃はキーボードより連射力しにくいため、大量のゾンビを全て倒すとなると指が疲れてしまいます。また、プレイヤーの3Dモデルが装備しているのは「自動小銃」と呼ばれる武器で、セミオートマチックとフルオートマチックを切り替えられるようになっています。

そこで、ゲームでも射撃モードを切り替えられるようにしましょう。
...
* 射撃モード変数、射撃間隔制御変数を作成.
* フルオートの場合、射撃間隔ごとに玉を発射. セミオートでは従来どおり.
* `C`キーで射撃モード切替.
* フルオート・セミオート画像を用意. 現在のモードに対応する画像を表示.
