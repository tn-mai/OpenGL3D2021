...
オフセットを計算するには、アクセッサが含まれるバッファビューを取得し、アクセッサとバッファビューのデータを組み合わせます。

オフセットは様々な場面で必要とされますが、その計算方法はすこし複雑です。そこで、オフセットを計算する関数を作成することにします。関数名は`GetBufferOffset`(ゲット・バッファ・オフセット)とします。

`ReadFile`関数の定義の下に、次のプログラムを追加してください。
