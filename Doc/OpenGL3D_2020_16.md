...
どんなルールがいいかは悩ましいところですが、今回はベロシティを使うルールにしようと思います。`CollideCylinders`関数を次のように変更してください。
...
+    // 差も0の場合はあきらめて+X方向に押し出す.
+    // ベロシティの差を計算.
+    const float vx = b.velocity.x - a.velocity.x;
+    const float vz = b.velocity.z - a.velocity.z;
+    float v = vx * vx + vz * vz; // 差の長さの2乗.
+    // 差の長さが0でなければ(vx, 0, vz)の方向に押し出す.
+    glm::vec3 n(1, 0, 0); // 押し出す方向.
+    if (v > 0) {
+      v = std::sqrt(v); // 「長さの2乗」を「長さ」に変換.
+      n = glm::vec3(vx / v, 0, vz / v); // 長さで割って正規化.
+    }
+    // アクターAとBを均等に押し返す.
+    a.position += n * r * 0.5f;
+    b.position -= n * r * 0.5f;
