...
多くのゲームでは、このような「特定の条件によってプログラムを実行する処理」を「フラグ(<ruby>旗<rt>はた</rt></ruby>)」という機能を使って管理しています。

フラグは主に`bool`型で表現され、`true`にすることを「フラグを立てる」、`false`にすることを「フラグを降ろす(または下げる)))」と言います。プログラムでは`if`文などを使って、
...
<strong>【課題01】</strong>
いくつかのゲームフラグに適当な説明文を設定してセーブしなさい。次にセーブしたマップデータを読み込み、説明文が正しく読み込めることを確認しなさい。
</pre>
...
選択したフラグ番号をセーブします。セーブ関数を次のように変更してください。
