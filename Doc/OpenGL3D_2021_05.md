...
さて、C++でファイルを操作するときは、そのファイルが「テキスト・ファイル」なのか「バイナリ・ファイル」なのかを指定しなければなりません。何も指定しなければ「テキスト・ファイル」として扱われます。バイナリ・ファイルとして扱いたいときは`std::ios_base::binary`を指定する必要があります。
...
+  const GLuint texGround = GLContext::CreateImage2D("Res/Ground.tga");
   const GLuint texTree = GLContext::CreateImage2D(5, 5, imageTree);
...
次のような、色も形もおかしくなった表示されると思います。プログラムを変更していないため、間違った情報をもとにテクスチャを作っているからです。
...
<strong>【課題01】</strong>
木のテクスチャ用の画像ファイルを作成し、<code>texTree</code>を作成した画像ファイルで置き換えなさい。
</pre>
...
地面と木だけではさびしいので、なにか建物を追加しましょう。とりあえず次のような長方形の建物を作ることにします。
...
<p align="center">
<img src="images/05_dice_net.png" width="33%" /><br>
</p>

注目してほしいのは、展開図の頂点数です。サイコロの頂点は`8`個なのに、展開図では`14`個に増えています。

展開図のように三次元の物体を二次元の平面で表現するためには、図形を切り開かなければなりません。上の展開図で同じアルファベットが振られた頂点は、張り合わせて三次元にすると同じ位置に重なります。

このように、三次元から二次元に変換する場合、いくつかの頂点は切り開かれて複数に分ける必要があります。その結果、頂点数が増えるというわけです。

複数に分けられた頂点は、それぞれが異なるテクスチャ座標を持つ必要があります。しかし、OpenGLの頂点データには、基本的にはひとつのテクスチャ座標しか割り当てられません。

そこで「テクスチャ座標の数だけ頂点を作る」という方法を使います。もちろん、これによって頂点データの数は増加します。しかし、テクスチャをきれいに貼り付けるためには、受け入れるだけの価値があります。

それでは、建物の頂点データを作りましょう。まずは展開図を考えます。これは次の画像のようになるでしょう。ちょっと理由があって上下反転していますが、今は気にしないでください。

<p align="center">
<img src="images/05_building_texture.png" width="80%" /><br>
[建物の展開図]
</p>

展開図の頂点数は12個で、各頂点には対応するインデックスを振っています。これを元にして、まずは画像データを作りましょう。プロジェクトの`Res`フォルダに`Building.tga`というファイルを追加してください(追加方法は2章を参照)。そして幅を`64`、高さを`16`に変更し、上の展開図とだいたい同じになるように色を塗って、保存してください。

展開図中の赤線は、どのように三角形に分割されるかを示すものなので、画像には描かないでください。

画像データを作成したら、次は頂点データを作成しましょう。建物は1階建て、大きさは`6x4m`、高さは`2m`とします。まずは座標データから作っていきましょう。`Main.cpp`を開き、木の座標データの下に次のプログラムを追加してください。
...
続いて、色データを追加します。建物の色はテクスチャで表現するので全て白にします。木の色データの下に、次のプログラムを追加してください。
...
最後にインデックスデータを追加します。木のインデックスデータの下に、次のプログラムを追加してください。
...
それでは、ここまでに追加したデータから描画データを作成しましょう。描画データリストに次のプログラムを追加してください。

```diff
 /// 描画データリスト.
 const Mesh::Primitive primGround(GL_TRIANGLES, 6, 0, 0); // 地面
 const Mesh::Primitive primTree(GL_TRIANGLES, 27, 6 * sizeof(GLushort), 4); // 木
+const Mesh::Primitive primBuilding(GL_TRIANGLES, 30, 33 * sizeof(GLushort), 13); // 建物
...
建物のインデックスデータは`30`個あるので、第2引数には`30`を指定します。

建物の頂点データは地面と木の次に定義しましたね。地面のインデックスデータ数が`6`、木のインデックスデータ数が`27`なので、建物のインデックスデータは`6+27=33`番目から始まります。つまり、第3引数は`33*sizeof(GLushort)`になります。

同様に、地面の頂点データ数は`4`、木の頂点データ数は`9`です。`4+9=13`なので、建物の頂点データは13番目から始まることになります。これで建物のデータの作成は完了です。
...
-  if (!texGround || !texTree) {
+  if (!texGround || !texTree || !texBuilding) {
     return 1;
   }
```
...
```diff
   // 後始末.
+  glDeleteTextures(1, &texBuilding);
...
プログラムが書けたらビルドして実行してください。これで家が表示されるはずです。
...
ところで、建物のテクスチャは上下反転して描いてもらいました。今からその理由を説明しましょう。実はですね、Visual Studioで作ったTGAファイルは「上下が逆さま」になっているんですよ！　おどろきの新事実ですね！　…え？　そうでもない？
...
プログラムが書けたらビルドして実行…と、その前に、`Building.tga`を上下反転して、正しい向きにしておきます。Visual Studioで`Building.tga`を開いてください。画像編集ウィンドウの上部アイコンリストの右端にある`▼`部分をクリックすると、メニューが表示されます。「ツール→上下反転」を選択すると画像の上下が反転します。

<p align="center">
<img src="images/05_vs_reverse_image_tool.png" width="50%" /><br>
</p>

編集した画像ファイルを保存したら、プログラムをビルドして実行してください。建物のテクスチャが以前と同じように表示されたら成功です。
...
<pre class="tnmai_assignment">
<strong>【課題02】</strong>
スマホなどのカメラを使って、地面テクスチャ用の画像を撮影しなさい。撮影した画像をVisual Studioで読み込み、TGAファイルとして保存しなさい。
</pre>

<pre class="tnmai_assignment">
<strong>【課題03】</strong>
課題02で作成したTGAファイルを、地面テクスチャとして表示しなさい。
</pre>
...
テクスチャは自由に作成して構いません。テクスチャ用の変数名は<code>texCube</code>にしてください。
<code>Primitive</code>データの変数名は<code>primCube</code>(プリム・キューブ)にしてください。
</pre>
