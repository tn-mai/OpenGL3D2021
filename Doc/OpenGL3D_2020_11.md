...
前回、タイトル画面とメインゲーム画面を作りました。しかし、2つの画面を行き来することはできませんでした。それは、各シーンは自分に必要な変数しか見ることができないからです。

そこで、シーンを連携させる仕組みを追加して、シーンを切り替えられるようにしていきます。

まずシーンを連携させるためのクラスを作りましょう。シーンを連携させるためには、連携させるシーンを管理する必要があります。そこで、クラス名は`SceneManager`(シーン・マネージャ)とします。日本語にすると「シーン管理者」ですかね。

<p align="center">
<img src="images/11_scene_manager_guy.png" width="15%" />[シーン管理者のイメージ]<br>
</p>
...
`SceneManager`は複数のシーンを管理するので、何らか方法で各シーンを識別できないと困ります。そこで、シーンごとに固有の名前を付けることにします。とりあえず「シーン名＝クラス名」としましょう。そして、シーンを切り替えるときは、シーン名を指定することにします。

また、通常は複数のシーンが同時に動くことはありません。そこで、動かす必要のあるシーンだけを作成し、別のシーンに切り替えるときは不要になったシーンを削除するようにします。シーンの作成と削除を行うために、シーンはポインタで管理することになるでしょう。

シーンの初期化・更新・描画は、`TitleScene`や`MainGameScene`の対応するメンバ関数を呼び出すことで行います。シーンの初期化は「シーンの切り替え」で行えばいいように思えます。

しかし、「シーンの切り替え」指示は「誰が」「いつ」出すのでしょうか？
...
存在しないデータを読み書きするのは、プログラミングにおいて最も危険な行為のひとつです。そこで、「シーン切り替え」を指示する段階では「切り替えの予約」だけを行うことにします。

実際の切り替えは「シーンを切り替えても安全な場所」、例えば「「実行中のシーンの更新」の手前」などで行います。
...
シーンの名前は`#define`で定義しておきます。これは`Visual Studio`の入力支援機能を使うためです。単なる文字列では入力支援を受けられませんが、マクロ定数にしておけば受けられます。インクルード文の下に、次のプログラムを追加してください。
...
次に、メンバ関数を宣言します。更新関数と描画関数と、それからシーンを切り替える関数が必要です。また、後述する「シングルトン」のための`Get`(ゲット)メンバ関数を追加します。`public`メンバに次のプログラムを追加してください。
...
次にコンストラクタ、デストラクタなどを宣言します。後述する「シングルトン」のために、これらは`private`メンバとして宣言します。`private`メンバに次のプログラムを追加してください。

```diff
   void ChangeScene(const std::string&);

 private:
+  // クラスの変数を作れなくするため、コンストラクタとデストラクタをprivateにする.
+  SceneManager() = default;
+  ~SceneManager() = default;
+  // コピーを禁止するため、コピーコンストラクタとコピー代入演算子をdelete指定する.
...
最後にメンバ変数を定義します。クラスの中でしか使わないので`private`メンバとして定義します。コピー代入演算子の定義の下に、次のプログラムを追加してください。
...
これでシーンマネージャクラスの定義は完成です。

### 1.3 シングルトン

コンストラクタとデストラクタを`private`にすると、そのクラスの変数を勝手に作ったり削除することができなくなります。加えて、コピーコンストラクタとコピー代入演算子を `delete` 指定しているので、コピーと代入も禁止されます。

作成とコピーを禁止している理由は、シーンを管理する変数をいくつも作れてしまうと、プログラムが混乱するだけだからです。

`SceneManager`のように「必要な変数はひとつだけで十分」という場合、`private`や`delete`によって変数を勝手に作れないようにしておき、唯一の変数はスタティックメンバ関数で管理するという方法が使われます。

このような、何らかの方法で、作成できる変数をひとつだけに制限する仕組みのことを「シングルトン」といいます。
...
このようなクラスは、「オブジェクト」や「アクター」、または「エンティティ」などと呼ばれています。本テキストでは「アクター」という名前を採用することにします。アクターは「<ruby>俳優<rt>はいゆう</rt></ruby>」や「<ruby>役者<rt>やくしゃ</rt></ruby>」という意味です。
...
同様に2行目は「`ActorList`(アクターリスト)と書いてあったら `std::vector<ActorPtr>` と書いてあるものとみなす」という指示になります。また、 `ActorPtr` は<br>
&nbsp;&nbsp;`std::shared_ptr<Actor>`<br>
と解釈されますので、最終的には<br>
&nbsp;&nbsp;`std::vector<std::shared_ptr<Actor>>`<br>
と書いたのと同じことになります。

>**【typedefではだめなんですか？】**<br>
>`typdef`でも問題はありません。それなのになぜ`using`構文が追加されたのかと言うと、`typedef`は型が複雑になると理解しにくくなりがちなので、バグの原因になることが多かったからです。
>特にC++では「テンプレート」という機能によって、非常に複雑な型を定義できるようになったため、定義が理解しやすいことは重要だったのです。

### 2.6 アクター用のファイルをダウンロードする

せっかくなので、アクター用の3Dモデルとテクスチャを用意しておきました。ウェブブラウザで<br>
`https://github.com/tn-mai/OpenGL3D2020/tree/master/Res`<br>
にアクセスし、以下のファイルをダウンロードしてプロジェクトの`Res`フォルダに保存してください。

* zombie_male.tga
* zombie_male_walk_0.obj

TGAファイルは右下の`Download`(ダウンロード)ボタンをクリックしてダウンロードします。

ただ、OBJファイルの場合はダウンロードボタンが表示されません。以下の画像を参考に、 `Raw` (ロー)ボタンを右クリックし(①)、表示されたメニューから「名前をつけてリンク先を保存」をクリックしてダウンロードしてください(②)。
...
それでは乱数を初期化して、ゾンビを出現させましょう。`Initialize`メンバ関数に次のプログラムを追加してください。
...
>`mt19937`は疑似乱数ではありますが、出力される値は真の乱数に非常に近いものです。これは良いことなのですが、ゲームで使うにはちょっと困ったこともあるのです。
