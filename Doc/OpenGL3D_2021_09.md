...
次に、弾アクターを作成するプログラムに、ベロシティを設定するプログラムを追加してください。
...
`vector`型の配列から要素を削除するには`erase`(イレース)メンバ関数を使います。イレース関数の引数には「イテレータ型」の値を指定します。

<pre class="tnmai_code"><strong>【書式】</strong>
削除した要素の次の要素を指すイテレータ vector::erase(削除する要素を指すイテレータ);
</pre>

「イテレータ」は、C++標準ライブラリにおいて「配列要素の位置を示すために使われる型」の<ruby>総称<rt>そうしょう</rt></ruby>です。ポインタをより汎用的にしたものだと考えてください。

`begin`(ビギン)メンバ関数は先頭要素を指すイテレータを返します。`i`番目の要素を指すイテレータを作るには、先頭要素を指すイテレータに`i`を足します。イレース関数とビギン関数を組み合わせることで、「`i`番目の要素を削除する」というプログラムを作ることができます。

<pre class="tnmai_code"><strong>【書式】</strong>
先頭要素を指すイテレータ vector::begin();
</pre>

また、イレース関数でアクターを削除すると、`i`は「次のアクター」を指した状態になります。そのままインクリメントすると`i`は「次の次のアクター」を指すことになり、「次のアクター」が処理されません。そこで、`i`をデクリメントして、インクリメントしたときに「次のアクター」を指すようにしています。

プログラムが書けたらビルドして実行してください。ボタンを押し続けても、あまりカクカクしなければ成功です。
...
これでT-34戦車と弾を衝突させる準備が整いました。最後に、プロセスコリジョン関数を使って衝突を判定しましょう。アクターの状態を更新するプログラムの下に、次のプログラムを追加してください。
...
+              // 弾を消去する
+              actors.erase(actors.begin() + b);
+              --b; // インクリメントしたとき次のアクターを指すように調整
+              // T-34戦車を消去する
+              actors.erase(actors.begin() + a);
+              --a; // インクリメントしたとき次のアクターを指すように調整
+              --b; // インクリメントしたとき次のアクターを指すように調整
+            }
+          }
+        }
