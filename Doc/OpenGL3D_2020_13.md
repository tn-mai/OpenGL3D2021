...
今回は、今後の下準備として、描画に関する変数をまとめたり、アニメーションをもっと簡単に扱えるようにしていきます。
...
最後に、`Render`メンバ関数を`GameData`クラスの変数を使うように変更します。`Render`メンバ関数に次のプログラムを追加してください。
...
1の方法は、計算時間はかかりますが、変数を追加する必要がないのが利点です。
